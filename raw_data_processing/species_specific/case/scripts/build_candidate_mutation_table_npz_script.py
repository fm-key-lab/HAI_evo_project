...
def main(path_to_p_file, path_to_sample_names_file, path_to_outgroup_boolean_file, path_to_list_of_quals_files, path_to_list_of_diversity_files, path_to_candidate_mutation_table, flag_cov_raw_sparse_matrix,flag_cov_norm_sparse_scale_matrix):
    ...
    ## NOTE: coverage_stats has indexes 0-9 with num sites that have  >= 1x, 2x, 3x ... 10x covg 
    ## NOTE: coverage_stats has index 10 with median coverage across all sites
    ## NOTE: coverage_stats has index 11 with mean coverage across all sites
    ## NOTE: coverage stats has index 12 with standard deviation of coverage across all sites
    coverage_stats=np.zeros((numSamples,13),dtype='uint') 
    for index in range(numSamples):
        current_array=all_coverage_per_bp[index]
        counts_for_output=[0]*10
        for output_index,val in enumerate(range(1,11)):
            to_sum_indices=np.where(current_array>=val)
            counts_for_output[output_index]=to_sum_indices[0].size
        coverage_stats[index,0:10]=counts_for_output
        coverage_stats[index,10]=np.median(current_array)
        coverage_stats[index,11]=np.average(current_array)
        coverage_stats[index,12]=np.std(current_array)

    ## Save cmt!   
    with gzip.open(path_to_candidate_mutation_table, 'wb') as f: 
        pickle.dump([SampleNames, p, counts, Quals, in_outgroup, indel_counter, coverage_stats], f,protocol=4) # protocol=4 for storage of files >4gb
