...
import subprocess
# from pylab import * ?? is matplotlib.pyplot sufficient?
...
def read_candidate_mutation_table_pickle_gzip(file_cmt_pickle_gz, indels = False):
    ...
    with gzip.open(file_cmt_pickle_gz, 'rb') as f:
        ...
        if indels:
            ...
            indel_size = np.array(cmt[10])
    if indels:
        return [quals,p,counts,in_outgroup,sampleNames,indel_counter,coverage_stats,indel_p,indel_depth,indel_support,indel_size]
    else:
...
def save_cutoff_vals(timestamp, filter_parameter_sample_across_sites, filter_parameter_site_per_sample, filter_parameter_site_across_samples, promotersize):

    with open(timestamp + '_cutoff_values.csv', 'w') as csv_file:  
        writer = csv.writer(csv_file)
        for key, value in filter_parameter_sample_across_sites.items():
            writer.writerow(['filter_parameter_sample_across_sites', key, value])
        for key, value in filter_parameter_site_per_sample.items():
            writer.writerow(['filter_parameter_site_per_sample', key, value])
        for key, value in filter_parameter_site_across_samples.items():
            writer.writerow(['filter_parameter_site_across_samples', key, value])
        writer.writerow(['promotersize', '', promotersize])
...
def read_latest_cutoff_vals(working_directory):
    ...
    with open(latest_cutoff_file, 'r') as fo: 
        ...
        filter_parameter_sample_across_sites = {}; filter_parameter_site_per_sample = {}; filter_parameter_site_across_samples = {}; promotersize = np.nan
        ## loop over lines of file and extract 
        for line in fo:
            line = line.strip().split(',')
            if line[0] == 'filter_parameter_sample_across_sites':
                if '.' in line[2]:
                    filter_parameter_sample_across_sites[line[1]] = float(line[2])
                else:
                    filter_parameter_sample_across_sites[line[1]] = int(line[2])
            elif line[0] == 'filter_parameter_site_per_sample':
                if '.' in line[2]:
                    filter_parameter_site_per_sample[line[1]] = float(line[2])
                else:
                    filter_parameter_site_per_sample[line[1]] = int(line[2])
            elif line[0] == 'filter_parameter_site_across_samples':
                if '.' in line[2]:
                    filter_parameter_site_across_samples[line[1]] = float(line[2])
                else:
                    filter_parameter_site_across_samples[line[1]] = int(line[2])
            elif line[0] == 'promotersize':
                promotersize = int(line[2])
    return [filter_parameter_sample_across_sites, filter_parameter_site_per_sample, filter_parameter_site_across_samples, promotersize]
        ...
        if mut_genenum[i] == int(mut_genenum[i]): # intragenic
            ...
            if p_anno.loc['type'] == 'CDS': ## check if CDS --> otherwise it is ncRNA or regulatory region (i.e. rRNA, tRNA, tmRNA, ncRNA, CRISPR, oriT, oriC, regulatory region,...)
                ...
                if len(mut_annotations['AA_gt_alt'])>0:
                    mut_annotations['muts'] = [refAA+str(mut_annotations['aa_pos'])+derAA for derAA in mut_annotations['AA_gt_alt'] if derAA != refAA]
...
def deduplicate_annotation_mutations(annotation_mutations, timestamp = '', association_order = ['N', 'S', 'R', 'P', 'I', 'U'], columns_to_reduce = ['nt_alt', 'AA_gt_alt', 'type', 'muts', 'NonSyn_rel_ref', 'NonSyn_rel_anc']):
    ...
    if dudup_p_anno_mut.empty:
        print('All positions are unique. No overlapping genetic regions contain any mutation')
            ...
            if sum(num_mutation_types > 1) == 1: ## only one entry has more than 1 mutational type --> select
                ...
                for i in range(max(num_mutation_types)): ## loop over every mutation type and select the one with the higher association
                    ...
                    if len(highest_prio_idx) > 1: ## multiple genetic regions have same mutational type priority 
                        print(f'On chromosome {chr} and position {pos} the same mutation type ({dedup_p_anno_mut_sub_prio.loc[0, "type"][i]}) for allele {dedup_p_anno_mut_sub_prio.loc[0, "nt_alt"][i]} was identified.')
                        print(f'Genes involved: {"; ".join(dedup_p_anno_mut_sub_prio.loc[highest_prio_idx, "gene"].values)}')
                        if all(dedup_p_anno_mut_sub_prio.loc[highest_prio_idx, 'muts'] != '.'): ## we have just nonsyn mutations --> try to select premature stops over other nonsyn mutations
...
def annotate_indels(annotation_genes , p_candidate_indels_gp , indel_size, hasindel_gp , min_indel_GL_diff , promotersize, ref_genome_folder):
    ...
    '''
    # p_candidate_indels_gp: genomic position of candidate indels
    # p_candidate_indels_gp: indel_p[candidate_indels]
    # indel_size=indel_size[candidate_indels,:]
    # indel_support = indel_support[candidate_indels,].flatten() 
    # ref_genome_folder=REFGENOMEFOLDER

    [chrstarts, genomelength,scafnames]= genomestats(ref_genome_folder);
    [locus_tag,chr_tag] = genomic_position_all(annotation_genes, genomelength, chrstarts); #loc_tag numbers all genes across genome (intergenic:0.5); chr_tag: numbers genes per chr (intergenic: #chr+0.5)
    nts = ['A','T','C','G'] #'atcg'; ## just for figuring out if underfined codon
    rc = ['T','A','G','C']  # 'tagc'; ## just for figuring out if underfined codon
    mut_genenum = chr_tag[p_candidate_indels_gp]
    loc_tag_p_gp = locus_tag[p_candidate_indels_gp]
    pos_chr = p2chrpos(p_candidate_indels_gp,chrstarts) # get two-col table chr,pos   
    lod_mutAnno = []
    for i,pos in enumerate(p_candidate_indels_gp):
        #print(i,pos)
        mut_annotations_helper = {}
        mut_annotations = {}
        mut_annotations['gene_num'] = mut_genenum[i]
        mut_annotations['gene_num_global'] = loc_tag_p_gp[i]
        mut_annotations['chr'] = pos_chr[i][0]
        ...
        mut_annotations['min_indel_GL_diff'] = min_indel_GL_diff[i]
        p_chr = pos_chr[i][0]
        indel_size_gt=[]
        for j,indel_callidx in enumerate(indel_size[i,:]):
            # i is index of current pos, j is index of samples 
            if hasindel_gp[i,j] == True :
                indel_size_gt.append(indel_size[i,j])
        mut_annotations['indel_size'] = list(dict.fromkeys( indel_size_gt ))
        if mut_genenum[i] == int(mut_genenum[i]): # intragenic
            p_anno = annotation_genes[p_chr-1].iloc[int(mut_genenum[i])-1] # both -1 necessary bcs list of df and df 0-based
            mut_annotations['product'] = p_anno.loc['product']
            mut_annotations['gene'] = p_anno.loc['gene']
            ...
            mut_annotations['translation'] = p_anno.loc['translation']
            if p_anno.loc['strand'] == 1: # get position within gene, consider strandedness
                mut_annotations['indel_pos'] = mut_annotations['pos'] - mut_annotations['loc1']+1 # pos/loc1 1-based. nt_pos 1-based. +1 to get 1-based nt_pos in gene (checked)!
            elif p_anno.loc['strand'] == -1:
                mut_annotations['indel_pos'] = p_anno.loc['loc2'] - mut_annotations['pos'] +1 # pos/loc2 1-based. +1 to get 1-based nt_pos in gene (checked)!
            else:
                mut_annotations['indel_pos'] = "." # can happen. eg. Crispr
            if mut_annotations['indel_pos'] == ".": # Observed with special 'type's like crispr annotated. rare! leads to no AA inference.
                aan = 9999999
            else:
                aan = int( (mut_annotations['indel_pos']-1 )/3 ) + 1 # get #codon that harbours mutation. 1-based.
                ncn = mut_annotations['indel_pos'] - ((aan-1)*3) # get #nucl within triplett. 1-based
                mut_annotations['aa_pos'] = aan            
            # impact of indel
            #NOTE: eventually extend this to parse the nts in the insertion/deletion to figure out impact
            #fixed mutation
            mut_annotations['indel_impact_protein_len']=[]
            mut_annotations['indel_impact_descriptive']=[]
            ## NOTE: current modification adds dummy GGGGGG.... sequence for indel of any size (should never create STOP codon by accident)
            for size in mut_annotations['indel_size']:
                if not np.isnan(size):
                    size=int(size)
                    if size % 3 == 0:
                        mut_annotations['indel_impact_protein_len'].append(0)
                        mut_annotations['indel_impact_descriptive'].append("inframe")
                    else:
                        sequence_original=mut_annotations['sequence']
                        translation_original=sequence_original.translate()
                        loc_original_stop=translation_original.find("*")
                        if size < 0: ## deletion, find if premature stop
                            sequence_indel=sequence_original[:mut_annotations['indel_pos']]+sequence_original[mut_annotations['indel_pos']+(size*-1):]
                        else: ## insertion, see note above, all inserted nts are G
                            sequence_indel=sequence_original[:mut_annotations['indel_pos']]+Seq('G'*size)+sequence_original[mut_annotations['indel_pos']:]
                        translation_updated=sequence_indel.translate()
                        loc_updated_stop=translation_updated.find('*')
                        if loc_original_stop == -1:
                            mut_annotations['type'] = 'U' ## no stop codon in original seq (pseudogene?)
                            indel_impact = np.NaN
                        elif loc_updated_stop == -1:
                            indel_impact = 0
                            mut_annotations['indel_impact_descriptive'].append("indefinite")
                        else:
                            indel_impact = loc_updated_stop - loc_original_stop
                            if indel_impact < -1:
                                mut_annotations['indel_impact_descriptive'].append("truncation")
                            else: mut_annotations['indel_impact_descriptive'].append("truncation, 1 AA")
                        mut_annotations['indel_impact_protein_len'].append(indel_impact)
        else: #intergenic
            if int(mut_genenum[i])>0: # get info for gene prior SNP (if any)
                p_anno = annotation_genes[p_chr-1].iloc[int(mut_genenum[i])-1] # both -1 necessary bcs list of df and df 0-based
                mut_annotations['gene1'] = p_anno.loc['gene']
...
def generate_tree(calls_for_tree,treeSampleNamesLong,sampleNamesDnapars,refgenome,filetag,buildTree=False,writeDnaparsAlignment=False,root_on_smpl='',coloring_pattern='',additional_raxml_parameters='--threads 2'):
    ...
    if buildTree=='PS':      
        ...
        subprocess.run(["raxml-ng --msa " + timestamp+".fa" + " --model GTR+G --prefix " + timestamp+"_ML " + additional_raxml_parameters],shell=True)
    return timestamp
...
def plot_coverage_fwd_rev_stacked(chr_pos_gp,annotation_mutations,lod_fwd_cov,lod_rev_cov,timestamp,subject="",no_processes=multiprocessing.cpu_count()-1):
    ...
    # ## create pool of processes (number of processes should nnot be greater than number of CPUs-1 avalable)
    # pool = multiprocessing.Pool(processes = no_processes)
    # ## create iterable for processes
    # i = range(chr_pos_gp.shape[0])
    # ## link function with inputs (without iterable) 
    # link_func_var = partial(plot_input, chr_pos_gp, annotation_mutations, lod_fwd_cov, lod_rev_cov)
    # ## start multiprocesses 
    # pool.map(link_func_var, i)
    # pool.close()
    # pool.join()
        ...
        for i,row in this_locustag_annotation_mutations.iterrows():
            ...
            if row['NonSyn']:
                ...
                if 'num_mutational_events' in this_locustag_annotation_mutations.columns:
                    ...
                    cand_muts_S += 1
        prob_obsN_excess = stats.binom_test(cand_muts_N, n=(cand_muts_N+cand_muts_S), p=dn_prob_per_gene[locustag], alternative='greater')
        prob_obsS_excess = stats.binom_test(cand_muts_S, n=(cand_muts_N+cand_muts_S), p=(1-dn_prob_per_gene[locustag]), alternative='greater')
...
def parallel_evo_module(goodpos2use,contig_positions,annotation_mutations, annotation_genes, params_dict ,plot=True,ortholog=True,plot_title=None, count_multiply_mut_pos=True, dn_ds_only_cand_genes=True,return_prob_sim_cand_genes=False, testing=False, quiet=False, exclude_adj_muts = False):
    ...
    # annotation_mutations = annotation_mutations_sb
...
def plot_molecular_clock(input_data,hdr,mean_count_per_visit,regress_dict,basescale,subject_identifier, jitter_x = 0, jitter_y = 0, title_suffix = ''):
    ...
    plt.scatter(data_df['colonization_days'] - jitter_x_array,
                (data_df['rate']*basescale) - jitter_y_array,
                facecolors='none', edgecolors='grey',alpha=.5)
    plt.scatter(mean_count_per_visit_d, mean_count_per_visit_vals, marker="x", s=100, c='black')
    plt.plot(xpos,(intercept*basescale) + slope*(xpos)*basescale,'b-')
    plt.ylim((0, useylim))
    plt.suptitle(subject_identifier+'\nMolecular rate (mut/y/'+str(basescale_mb)+'Mb) = '+str( np.round(365*slope*basescale ,3) ) + '\n r^2 = '+str( np.round(r_value_sq ,3) ), fontsize=12)
    plt.xlabel("Time (days)",fontsize=14)
    plt.ylabel("Observed mutations per "+str(basescale_mb)+'Mb',fontsize=14)    
    plt.tight_layout()
    subprocess.run(["mkdir -p pdf/molclock/ "],shell=True) # build output folder
    plt.savefig(f"pdf/molclock/{subject_id}_assembleBased{title_suffix}.pdf")
    plt.close()
    print('Molecular clock analysis: slope: ' + str(np.round(365*slope*basescale ,3)) + ", r^2: " + str( np.round(r_value_sq ,3) ) )
    print("Done. pdf/molclock/" + subject_id + "_assembleBased{title_suffix}.pdf")
...
def findrecombinantSNPs(p, mutantAF, distance_for_nonsnp, corr_threshold_recombination):
    ''' Mutations that  covary in close physical proximity (unexpected by chance) are likely non-independent mutations due to HGT event '''
...
def plot_minorAF_rates(minorAF, hasmutation, sampleNames, subject, refgenome, timestamp, titlelabel = ''):
    ...
    for l_bound, u_bound in zip(lower_bounds, upper_bounds):
        freq_minorAF[u_bound] = np.sum(((minorAF > l_bound) & (minorAF <= u_bound)) & hasmutation, axis = 0) / np.sum(hasmutation, axis = 0)

    # freq_minorAF_0_1 = np.sum(((minorAF > 0) & (minorAF < 0.01)) & hasmutation, axis = 0) / np.sum(hasmutation, axis = 0)
    # freq_minorAF_1_2 = np.sum(((minorAF >= 0.01) & (minorAF < 0.02)) & hasmutation, axis = 0) / np.sum(hasmutation, axis = 0)
    # freq_minorAF_2_3 = np.sum(((minorAF >= 0.02) & (minorAF < 0.03)) & hasmutation, axis = 0) / np.sum(hasmutation, axis = 0)
    # freq_minorAF_3_4 = np.sum(((minorAF >= 0.03) & (minorAF < 0.04)) & hasmutation, axis = 0) / np.sum(hasmutation, axis = 0)
    # freq_minorAF_4_5 = np.sum(((minorAF >= 0.04) & (minorAF < 0.05)) & hasmutation, axis = 0) / np.sum(hasmutation, axis = 0)
    # freq_minorAF_5_10 = np.sum(((minorAF >= 0.05) & (minorAF < 0.10)) & hasmutation, axis = 0) / np.sum(hasmutation, axis = 0)
    # freq_minorAF_10_all = np.sum((minorAF >= 0.10) & hasmutation, axis = 0) / np.sum(hasmutation, axis = 0)
...
def plot_quals_rates(quals, hasmutation, sampleNames, subject, refgenome, timestamp, titlelabel = ''):
    ...
    freq_quals = {}
    for l_bound, u_bound in zip(lower_bounds, upper_bounds):
        freq_quals[u_bound] = np.sum(((quals < u_bound) & (quals >= l_bound)) & hasmutation, axis = 0) / np.sum(hasmutation, axis = 0)

    # freq_quals_all_200 = np.sum((quals >= 200) & hasmutation, axis = 0) / np.sum(hasmutation, axis = 0)
    # freq_quals_200_100 = np.sum(((quals < 200) & (quals >= 100)) & hasmutation, axis = 0) / np.sum(hasmutation, axis = 0)
    # freq_quals_100_30 = np.sum(((quals < 100) & (quals >= 30)) & hasmutation, axis = 0) / np.sum(hasmutation, axis = 0)
    # freq_quals_30_0 = np.sum((quals < 30) & hasmutation, axis = 0) / np.sum(hasmutation, axis = 0)
