...
'''
MF 2022/12: 
This script was adapted from SRST2 https://github.com/katholt/srst2 
to be used within snakemake workflows to download necessary mlst data.
'''
...
def parse_args():
	...
	parser.add_argument('--output_path',
						default = False,
						help = 'Directory where to download MLST data for required species')
...
def getSpeciesInfo(species_node, species, exact):
	species = species.replace('_', ' ') ## replace underscores in speciesname for matching with mlst DB
...
def main():
	...
	outpath_ext = outpath + species_name_underscores + '/'
	# Remove any Bowtie/Samtools index files that already exist.
	for filename in glob.glob(outpath_ext + species_name_underscores + '*.bt2'):
		os.remove(filename)
	for filename in glob.glob(outpath_ext + species_name_underscores + '*.fai'):
		os.remove(filename)

	# output information for the single matching species
	species_all_fasta_filename = outpath_ext + species_name_underscores + '.fasta'
	...
	log_file = open(outpath_ext + log_filename, "w")
	...
	profile_file = open(outpath_ext + profile_filename, 'w')
