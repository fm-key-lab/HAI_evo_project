...
parser = argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter,
                                 description='''\
    Infer orthologs across two or more bakta-based annotations, and returns overview table for all genes.
...
def read_merge_sample_annotation_file(file_sample_annotation):
    # get list of annotation file paths, tuple/dict of subjectID and bakta-assigned gene-tag (use first line)
    subj_tag_dict = {}
...
#        with open(file_sample_annotation,'r') as infile:
        for line in file_sample_annotation:
            ...
            print('1')
            print(line)
            # read annotation file: extract bakta-gene-tag (from 1st line) and merge all annotation files into one
            with open(line[1]) as annofile:
                # link subjectID with bakta gene tag
                first_line = annofile.readline()
                if first_line.startswith('>'):
                    bakta_gene_tag = first_line.strip().split(' ')[0].split('>')[1].split('_')[0]
                    subj_tag_dict[ line[0] ] = bakta_gene_tag
                else:
...
def read_cdhit_cluster(cdhit_clstr,percentIdentity,bakta_tag_list):
    ...
    saab_cluster_genes = {}
#    value_default = ['NA' for i in bakta_tag_list]
    with open(cdhit_clstr) as infile:
        for line in infile:
            if line.startswith('>'):                
                cluster_tag = "SAAB_" + "%05d" % ctr + "_pid" + percentIdentity + "_" + rdm_tag
                saab_cluster_genes[cluster_tag] = ['NA' for i in bakta_tag_list]
                ctr += 1
            else:
                line = line.strip().split('\t')[1].split(" ")[1].split('.')[0].split('>')[1] # remodel string so it matches bakta-gene_id eg. "0       10155aa, >JKPBNODD_00001... *"
                subject_identifier_bakta = line.split('_')[0]             
                subject_index = bakta_tag_list.index( subject_identifier_bakta )
...
''' MAIN '''
# TEST Vars
#file_sample_annotation = "/Users/fmk/Documents/mit/stapAD/tmp/pycode/bakta_ffn/subject_4_9_16.list"
...
#    filetype = args.filetype
    ...
    # get concatenated annotation file (output: merged_annotation.fa) and dict[subject]=bakta-tag
    subj_tag_dict = read_merge_sample_annotation_file(file_sample_annotation)
    subject_list_ord = list(subj_tag_dict.keys())
    baktaTag_list_ord = [ subj_tag_dict[k] for k in subject_list_ord ]
    ...
    cdhit_res_dict = read_cdhit_cluster(outpath+"cdhit_results.clstr",percentIdentity,baktaTag_list_ord)
